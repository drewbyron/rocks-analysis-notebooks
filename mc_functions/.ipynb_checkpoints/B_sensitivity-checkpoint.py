...
def AUC_expectation_alt(set_fields_ne, set_fields_he, set_fields_niave, freq_BWs, b = 0, plot = False): 
    
    integrate_vect = np.vectorize(integrate.quad)
    
    energy_acceptances_high_ne = sc.freq_to_energy(
        freq_BWs[:,0], set_fields_ne
    )
    energy_acceptances_low_ne = sc.freq_to_energy(
        freq_BWs[:,1], set_fields_ne
    )
    energy_acceptances_ne = np.stack((energy_acceptances_low_ne, energy_acceptances_high_ne), axis=-1)
    
    energy_acceptances_high_he = sc.freq_to_energy(
        freq_BWs[:,0], set_fields_ne
    )
    energy_acceptances_low_he = sc.freq_to_energy(
        freq_BWs[:,1], set_fields_ne
    )
    energy_acceptances_he = np.stack((energy_acceptances_low_he, energy_acceptances_high_he), axis=-1)
    
    # Now write these (in principle different) energy acceptances to a dict.  
    energy_acceptances = {"Ne19":energy_acceptances_ne, 
                         "He6":energy_acceptances_he }
...
def field_spread_MC(set_fields_ne, 
                  set_fields_he, 
                  set_fields_niave,
                  freq_BWs, 
                  C_exp, 
    ...
    # Simulate data.
    ratio_pred = AUC_expectation_alt(set_fields_ne, set_fields_he, set_fields_niave, freq_BWs, b = b, plot = False)
...
def run_B_sensitivity_test( field_errs, field_offsets, trial_max = 20 ):
    ...
    for field_err in field_errs: 
        ...
        for trial in range(trial_max):
            ...
            set_fields_ne = rng.normal(set_fields_niave+set_fields_niave*field_offset, set_fields_niave*field_err)
            set_fields_he = rng.normal(set_fields_niave, set_fields_niave*field_err)

            # Simulate simple experiment.
            ratio_exp, spectra_ne_exp, spectra_he_exp = field_spread_MC(set_fields_ne,
                                                                              set_fields_he,
                                                                              set_fields_niave,
                                                                             freq_BWs, 
                                                                             C_exp, 
            ...
            result = minimize(objfunc_chisq, my_pars, args = (freq_BWs, set_fields_niave, ratio_exp, b))
