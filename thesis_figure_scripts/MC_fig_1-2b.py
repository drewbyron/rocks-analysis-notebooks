...
ratio's sensitivity to the number of monitor events (X_{mon}). 
"""
# Imports.
import sys
import numpy as np
...
fig_name_counts = Path("MC_fig_1ab_counts.png")
fig_name_mon = Path("MC_fig_2ab_mon_.png")
fig_path_counts = fig_dir / fig_name_counts
fig_path_mon = fig_dir / fig_name_mon

# CSV paths.
N_bs_csv_path = fig_dir / Path("N_bs_b.csv")
N_bs_uncert_csv_path = fig_dir / Path("N_bs_uncert_b.csv")
mon_bs_csv_path = fig_dir / Path("mon_bs_b.csv")
mon_bs_uncert_csv_path = fig_dir / Path("mon_bs_uncert_b.csv")
...
# Select set fields.
set_fields = np.arange(1.75, 3.5, 0.25)
# print(len(set_fields), set_fields)
