...
def energy_domain_plot(ax0,ax1, gamma_acceptances, gamma_widths, gamma_heights, gamma_height_errs, SM_heights, ratio_corr, ratio_pred, label = True):
    if label: 
        ax0.bar(gamma_acceptances[:,0], gamma_heights.values, width=gamma_widths,yerr =gamma_height_errs.values,
                color='r', align = 'edge', label = "data (corrected)")
        ...
        ax1.plot(gamma_acceptances[:,0], (ratio_corr["Ne19_corr"] - ratio_pred["Ne19"])/ratio_corr["sNe19_corr"] , 
                 label= f"residuals", 
                 marker="o", 
                 ls = "None",
                 color = "r",
