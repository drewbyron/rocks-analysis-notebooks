...
def process_snr_study(
    ...
    for snr in snrs:
        ...
        ne_spectrum = re.build_spectrum(
            ne.events.copy(deep=True),
            ne.root_files.copy(deep=True),
            event_cuts,
            normed_cols=normed_cols,
        )
        ne_spectra.append(ne_spectrum)

        he = snr_study["he"][snr]
        he_spectrum = re.build_spectrum(
            he.events.copy(deep=True),
            he.root_files.copy(deep=True),
            event_cuts,
            normed_cols=normed_cols,
        )
...
def combine_ratios(snr_study, event_cuts, snrs, normed_cols):
    ...
    # Let the systematic error be the range/2.
    ratio_sys = (Ratio_all.max(axis=0) - Ratio_all.min(axis=0)) / 2
    ...
    ratio_combined["sRatio"] = (ratio_sys**2 + ratio_stat**2) ** 0.5
