...
def build_canditate_events(
    ...
    bspec = bs.BetaSpectrum(isotope, b=b)
    Ws = np.linspace(1.00, bspec.W0, int(1e4))
    pdf = bspec.dNdE(Ws)

    # Sample n gammas from full pdf.
    gammas = rng.choice(Ws, size=n, p=pdf / pdf.sum())

    # Generate expanded BW
    freq_BWs_generate = build_extended_freqBW(freq_BW_visible, set_fields, slew_time)

    energy_acceptances_high = sc.freq_to_energy(freq_BWs_generate[:, 0], set_fields)
    energy_acceptances_low = sc.freq_to_energy(freq_BWs_generate[:, 1], set_fields)
    ...
    for index, (gamma_acceptance, field) in enumerate(
        zip(gamma_acceptances, set_fields)
    ):

        gamma_field = gammas[
            np.logical_and(gammas >= gamma_acceptance[0], gammas <= gamma_acceptance[1])
        ]
    ...
    events["EventStartFreq"] = sc.energy_to_freq(
        sc.energy(events["gamma_start"]), events["set_field"]
    )
...
def cut_events_fn(events, freq_BW_visible):
    ...
    events["EventEndTime"] = np.minimum(
        events["EventEndTime"],
        events["EventStartTime"]
        + (freq_BW_visible[1] - events["EventStartFreq"]) / events["Slope"],
    )

    cut_condition = events["EventEndFreq"] < freq_BW_visible[0]
    events = events.copy().loc[~cut_condition,:]

    # Note that the order of the below two operations is crucial for it to work.
    events["EventStartTime"] = np.maximum(
        events["EventStartTime"],
        events["EventStartTime"]
...
def get_slope_info(events, freq_BW_visible, fix_slopes):
    ...
    if fix_slopes:
        ...
        events["Slope"] = sc.df_dt(
            energy_fixed, events["set_field"], events["power_start"]
        )
        ...
        events["power_start"] = power_larmor
        events["Slope"] = sc.df_dt(
            events["energy_start"], events["set_field"], events["power_start"]
        )

    events["energy_end"] = (
        events["energy_start"] - power_larmor * events["TimeLength"] * sc.J_TO_EV
    )
    ...
    events["SlopeEnd"] = sc.df_dt(
        events["energy_end"], events["set_field"], events["power_end"]
    )

    events["EventEndFreq"] = (
        events["EventStartFreq"] + events["TimeLength"] * events["Slope"]
    )
...
def build_MC_spectra(events_ne, events_he, cuts, monitor_rate=10**8, mon_pois=True):
    ...
    spectra_ne_exp["event_count"] = (
        ne_events_cut.groupby("set_field").EventStartFreq.count().values
    )
    spectra_he_exp["event_count"] = (
        he_events_cut.groupby("set_field").EventStartFreq.count().values
    )
...
def build_MC_spectra_alt(
    events_ne, events_he, cuts, monitor_rate=10**8, mon_pois=True
):
    ...
    spectra_ne_exp["event_count"] *= (
        ne_events_cut.groupby("set_field").detection_prob.mean().values
    )
    spectra_he_exp["event_count"] *= (
        he_events_cut.groupby("set_field").detection_prob.mean().values
    )
...
def build_from_below_ratio_pred(
    set_fields, freq_BW, slew_time, b, seed=12345, n_tot=1e6
):
    """Note that from experimentation you need at least n_tot = 1e6 for 
    seed independent predictions. 
    ...
    events_ne = build_canditate_events(
        freq_BW,
        set_fields,
        "Ne19",
        cut_events=True,
    ...
    events_he = build_canditate_events(
        freq_BW,
        set_fields,
        "He6",
        cut_events=True,
    ...
    cuts = {
        "EventStartFreq": (freq_BW[0], freq_BW[1]),
    }

    spectra_ne_exp, spectra_he_exp = build_MC_spectra(
        events_ne, events_he, cuts, monitor_rate=10**10, mon_pois=False
    )

    # Build the experimental ratio.
    ratio_pred = re.build_ratio(spectra_ne_exp, spectra_he_exp)
...
def objfunc_chisq_slewfree(my_pars, ratio_exp, set_fields, freq_BW):
    ...
    # Build the experimental ratio.
    ratio_pred = build_from_below_ratio_pred(set_fields, freq_BW, slew_time, b=b)

    # ratio_exp["sRatio"].iloc[-1]*=4
    chisq_gauss = (ratio_pred["Ratio"] - C * ratio_exp["Ratio"]) / (
        C * ratio_exp["sRatio"]
    )
    return chisq_gauss
