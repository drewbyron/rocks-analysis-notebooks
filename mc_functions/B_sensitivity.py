...
def AUC_expectation_alt(
    ...
    energy_acceptances_high_he = sc.freq_to_energy(freq_BWs[:, 0], set_fields_he)
    energy_acceptances_low_he = sc.freq_to_energy(freq_BWs[:, 1], set_fields_he)
...
def run_B_sensitivity_test(field_errs, field_offsets, trial_max=20):
    ...
    # first run field_errs test. This is introducing a gaussian spread to the true (physical)
    # field.
    ...
    for field_offset in field_offsets:
        field_err = 0
        field_offset_test_ne[field_offset] = []
        field_offset_test_he[field_offset] = []


        # reducing this to one trial because there is no randomness. 
        for trial in range(2):
            print(f"NE: Field offset: {field_offset}, trial: {trial}")

            # Add in some uncertainty in the field that's different for ne and he.
            set_fields_ne = set_fields_niave*(1 + field_offset)
            set_fields_he = set_fields_niave 
            ...
            field_offset_test_ne[field_offset].append(result.params["b"].value)


            print(f"HE: Field offset: {field_offset}, trial: {trial}")

            # Add in some uncertainty in the field that's different for ne and he.
            set_fields_ne = set_fields_niave
            set_fields_he = set_fields_niave*(1 + field_offset)
            ...
            my_pars = Parameters()
            my_pars.add("C", value=1, min=0, max=10, vary=True)
            my_pars.add("b", value=0.0, min=-10, max=10, vary=True)

            result = minimize(
                mc.objfunc_chisq, my_pars, args=(freq_BWs, set_fields_niave, ratio_exp)
            )

            field_offset_test_he[field_offset].append(result.params["b"].value)

    field_err_test = pd.DataFrame(field_err_test)
    field_offset_test_ne = pd.DataFrame(field_offset_test_ne)
    field_offset_test_he = pd.DataFrame(field_offset_test_he)

    return field_err_test, field_offset_test_ne, field_offset_test_he
