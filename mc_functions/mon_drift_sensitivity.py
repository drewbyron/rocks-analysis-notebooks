...
def mon_drift_MC(
    ...
    spectra_ne_exp["tot_monitor_rate"] = (
        ...
        * (
            1
            + spectra_ne_exp.reset_index().index
            * mon_drift_ne
            / spectra_ne_exp.reset_index().index.max()
        )
    )
...
def run_mon_sensitivity_test(mon_drifts, trial_max=20):
    ...
    for mon_drift in mon_drifts:
        ...
        for trial in range(trial_max):
            ...
            ratio_exp, spectra_ne_exp, spectra_he_exp = mon_drift_MC(
                set_fields,
                freq_BWs,
                C_exp,
                b,
            ...
            result = minimize(
                objfunc_chisq, my_pars, args=(freq_BWs, set_fields, ratio_exp, b)
            )
